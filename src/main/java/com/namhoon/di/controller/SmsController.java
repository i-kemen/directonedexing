...
public class SmsController {
    ...
    public SmsController() {
        ...
        this.messageService = NurigoApp.INSTANCE.initialize("NCSZQWUESTNSXBG9", "41NNZXGBT7N5HPOJJ9CQW82TH8PVYZBH", "https://api.coolsms.co.kr");
    }
...
//    public SingleMessageSentResponse sendVerification(@RequestParam("phoneNumber") String phoneNumber) {
//        Message message = new Message();
//
//        // 랜덤 인증번호 생성
//        String verificationCode = String.format("%06d", (int)(Math.random() * 1000000));
//
//        message.setFrom("01063909180"); // 고정된 발신번호
//        message.setTo(phoneNumber);     // 사용자가 입력한 번호
//        message.setText("[하나증권] 인증번호는 [" + verificationCode + "] 입니다.");
//
...
//    }
    ...
    @PostMapping("/send-verification")
    public SingleMessageSentResponse sendVerification(@RequestParam("phoneNumber") String phoneNumber, HttpSession session) {
        Message message = new Message();
        ...
        SingleMessageSentResponse response = this.messageService.sendOne(new SingleMessageSendingRequest(message));
        System.out.println(response);
        session.setAttribute("smsCertificationNumber", verificationCode);
        return response;
    }


    @PostMapping("/sms")
    public ResponseEntity<String> verifySmsCode(@RequestParam("phoneCodeInput") String userInputCode, HttpSession session) {
        String savedCode = (String) session.getAttribute("smsCertificationNumber"); // 세션에서 인증번호를 가져옵니다.
        if (savedCode != null && savedCode.equals(userInputCode)) {
            session.removeAttribute("smsCertificationNumber"); // 인증 성공 후 세션에서 인증번호 삭제
            return ResponseEntity.ok().body("Verification successful"); // 인증 성공
        } else {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Invalid verification code"); // 인증 실패
        }
    }
    ...
    public SingleMessageSentResponse sendMmsByResourcePath() throws IOException {
        ...
        message.setFrom("01015883111");
        message.setTo("01063909180");
