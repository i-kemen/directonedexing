...
<script>
    ...
    function refreshSelectedCheckboxCount() {
        const rightContainer = document.querySelector(".jogun-right");
        const containers = rightContainer.querySelectorAll(".checkbox-container");
        const selectedDiv = document.querySelector(".jogun-selected");

        if (containers.length > 0) {
            selectedDiv.textContent = containers.length;
        } else if (document.getElementById('nojogun').checked) {
            selectedDiv.textContent = '조건 선택 안함';
        } else {
            selectedDiv.textContent = '0';
        }
    }

    document.addEventListener("DOMContentLoaded", function() {
        const leftContainer = document.querySelector(".jogun-left");
        const rightContainer = document.querySelector(".jogun-right");
        const nojogunCheckbox = document.getElementById('nojogun');
        ...
        function resetAll() {
            ...
            refreshSelectedCheckboxCount();
        }

        leftContainer.addEventListener("click", function(e) {
            if (e.target.matches(".checkbox-container, .checkbox-container *")) {
                ...
                if (checkbox.checked) {
                    ...
                    const existing = Array.from(rightContainer.querySelectorAll(".checkbox-container input[type='checkbox']")).find(el => el.id === checkbox.id);
                    if (existing) {
                        existing.closest('.checkbox-container').remove();
                    }
                }
                refreshSelectedCheckboxCount();
            }

            if (e.target.matches("img[style*='reset.png'], img[style*='reset.png'] *")) {
                resetAll();
            }

            if (e.target === nojogunCheckbox || e.target.closest('#nojogun')) {
                resetAll();
                if (nojogunCheckbox.checked) {
                    const selectedDiv = document.querySelector(".jogun-selected");
                    selectedDiv.textContent = '조건 선택 안함';
                }
            }
    ...
    function getMetrics() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                sectorsWithIndustries = data.custom_index["Sectors with Industries"];
                console.log("Sectors with Industries:", sectorsWithIndustries);
...
// KOSPI 값 설정
    ...
    let sectorsWithIndustries = {};  // 이 변수에 'Sectors with Industries' 정보 저장
                ...
                pie: {
                    ...
                    events: {
                        click: function (event) {
                            console.log("Clicked Sector:", event.point.name);
                            // 클릭된 섹터의 이름을 가져옵니다.
                            let clickedSector = event.point.name;

                            // 해당 섹터의 데이터가 sectorsWithIndustries에 존재하는지 확인
                            if (!sectorsWithIndustries[clickedSector]) {
                                console.error(`No industries found for sector: ${clickedSector}`);
                                return;
                            }

                            // 해당 섹터에 속한 산업들의 목록을 가져옵니다.
                            let relatedIndustries = sectorsWithIndustries[clickedSector];

                            // 산업 차트를 업데이트하는 함수를 호출합니다.
                            updateIndustryPieChart(relatedIndustries);
                        }
                    }
                }
            },
    ...
    function updateIndustryPieChart(relatedIndustries) {
        console.log("Related Industries to Update:", relatedIndustries);
        if (!relatedIndustries || relatedIndustries.length === 0) return;

// 산업별 카운트를 증가시키는 코드를 제거
        let processedData = [];
        for (let industry of relatedIndustries) {
            processedData.push({
                name: industry,
                y: 1
            });
