...
<head>
    ...
    <style>
        ...
        * {
        ...
        .modal h2 {
            color: #E90061; /* Change according to your design preference */
            margin-bottom: 20px;
        }
    ...
    <div class="contents">
        <div class="index-top">
            <div class="top-header">
                ...
                <button class="edit2" onclick="showOptimizationModal()">포트폴리오 최적화</button>
            </div>
            <div class="top-main">
                <div class="top-left">
...
// 데이터를 테이블에 업데이트
            updateTable(targetData['Optimized Weights']);
...
// KOSPI 값 설정
    ...
    function updateTable(data) {
        ...
        Object.entries(data).forEach(function ([key, value], index) {
            if (rows[index]) {
                var weightCell = rows[index].cells[3];

                var weightValue;
                if (typeof value === 'string') {
                    weightValue = parseFloat(value.replace('%', ''));
                } else {
                    weightValue = parseFloat(value) * 100;
                }
                ...
                if (!isNaN(weightValue)) {
                    weightCell.textContent = weightValue.toFixed(2) + '%';
                    console.log("Calculated weightValue:", weightValue);
                } else {
                    console.error(`Invalid weight value for key ${key}: ${value}`);
                }
            }
        });
    }
    ...
    <%--                weightValue = parseFloat(value.replace('%', ''));--%>
    <%--            } else {--%>
    <%--                weightValue = parseFloat(value) * 100; // 숫자형 데이터에 100을 곱합니다.--%>
    <%--            }--%>
