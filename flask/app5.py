...
def send_metrics_to_springboot(custom_index_metrics, kospi_metrics, max_sharpe_metrics, min_volatility_metrics, risk_parity_metrics):
    try:
        metrics = {
...
def fetch_and_plot_data():
    ...
    global previous_data, kospi_data_cache
    tickers_sectors = {}
    tickers_industries = {}
    ...
    while True:
        ...
        try:
            ...
            if modified_kor_tickers != previous_data:
                ...
                # Max Sharpe 비중 추가
                metrics_max_sharpe['Optimized Weights'] = dict(zip(active_tickers, optimized_weights_max_sharpe.tolist()))

                # Min Volatility 비중 추가
                metrics_min_volatility['Optimized Weights'] = dict(zip(active_tickers, optimized_weights_min_volatility.tolist()))

                # Risk Parity 비중 추가
                metrics_risk_parity['Optimized Weights'] = dict(zip(active_tickers, optimized_weights_risk_parity.tolist()))
                ...
                # 메트릭 전송 부분 추가
                send_metrics_to_springboot(metrics, kospi_metrics, metrics_max_sharpe, metrics_min_volatility, metrics_risk_parity)
